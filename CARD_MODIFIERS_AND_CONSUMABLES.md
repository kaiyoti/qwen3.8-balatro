...
**APPROVED FOR IMPLEMENTATION.** (Note: this file was lost on disk at some
point after being written and before the git repo was initialized — it was
never committed. Recreated from the original content. If anything here
seems inconsistent with work already done, the working code on disk is the
...
Note: some of Part C may already be done by now (joker row layout, deck
pile, etc. per later requests) — check current state before assuming these
are still outstanding, only implement what's actually still missing.
